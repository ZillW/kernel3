...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
        ...
        pframe_t *allocated_pframe;
        int fill_operation_result;
        int lookup_retry_result;
        ...
        if (resident_pframe == NULL) {
                ...
                allocated_pframe = pframe_alloc(o, pagenum);
                if (allocated_pframe == NULL) {
                ...
                fill_operation_result = pframe_fill(allocated_pframe);
                if (fill_operation_result < 0) {
                        dbg(DBG_PRINT, "(GRADING3D)\n");
                        pframe_free(allocated_pframe);
                        return -EFAULT;
                }

                dbg(DBG_PRINT, "(GRADING3B)\n");
                *result = allocated_pframe;
                KASSERT(NULL != *result);
                KASSERT(!pframe_is_busy(*result));
                return fill_operation_result;
        } else {
                dbg(DBG_PRINT, "(GRADING3B)\n");
                
                while (pframe_is_busy(resident_pframe)) {
                        dbg(DBG_PRINT, "(GRADING3B)\n");
                        sched_sleep_on(&resident_pframe->pf_waitq);
                        resident_pframe = pframe_get_resident(o, pagenum);
                        if (resident_pframe == NULL) {
                                lookup_retry_result = pframe_get(o, pagenum, result);
                                return lookup_retry_result;
                        }
                }
                
                dbg(DBG_PRINT, "(GRADING3B)\n");
                *result = resident_pframe;
                KASSERT(NULL != *result);
...
pframe_pin(pframe_t *pf)
{
        ...
        already_pinned_flag = pframe_is_pinned(pf);
        if (!already_pinned_flag) {
                dbg(DBG_PRINT, "(GRADING3B)\n");
...
{
        int should_move_to_allocated_list;
        ...
        pf->pf_pincount--;
        should_move_to_allocated_list = (pf->pf_pincount == 0);
        
        if (should_move_to_allocated_list) {
                dbg(DBG_PRINT, "(GRADING3B)\n");
                list_remove(&pf->pf_link);
                list_insert_tail(&alloc_list, &pf->pf_link);
                nallocated++;
