...
{
        KASSERT(file); dbg(DBG_PRINT, "(GRADING2A 1.a)\n");
        KASSERT(S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)); dbg(DBG_PRINT, "(GRADING2A 1.a)\n");

        if (S_ISCHR(file->vn_mode)) {
                KASSERT(file->vn_cdev &&
                        file->vn_cdev->cd_ops &&
                        file->vn_cdev->cd_ops->read);
		dbg(DBG_PRINT, "(GRADING2A 1.a)\n");
                bytedev_t *dev = file->vn_cdev;
		dbg(DBG_PRINT, "(GRADING2B)\n");
                return dev->cd_ops->read(dev, offset, buf, count);
        }
        if (S_ISBLK(file->vn_mode)) {dbg(DBG_PRINT, "(GRADING2B)\n"); return -ENOTSUP;}
        return -EINVAL;
}
...
{
        KASSERT(file); dbg(DBG_PRINT, "(GRADING2A 1.b)\n");
        KASSERT(S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)); dbg(DBG_PRINT, "(GRADING2A 1.b)\n");

        if (S_ISCHR(file->vn_mode)) {
                KASSERT(file->vn_cdev &&
                        file->vn_cdev->cd_ops &&
                        file->vn_cdev->cd_ops->write);
...
{
	bytedev_t *the_dev = file->vn_cdev;
    KASSERT(the_dev != NULL && the_dev->cd_ops != NULL && the_dev->cd_ops->mmap != NULL);
    dbg(DBG_PRINT, "(GRADING3B)\n");
    return the_dev->cd_ops->mmap(file, vma, ret);
}
...
{
bytedev_t *the_dev = file->vn_cdev;
    KASSERT(the_dev != NULL && the_dev->cd_ops != NULL && the_dev->cd_ops->fillpage != NULL);
    dbg(DBG_PRINT, "(GRADING3B)\n");
    return the_dev->cd_ops->fillpage(file, offset, pagebuf);
}
...
{
bytedev_t *the_dev = file->vn_cdev;
    KASSERT(the_dev != NULL && the_dev->cd_ops != NULL && the_dev->cd_ops->dirtypage != NULL);
    dbg(DBG_PRINT, "(GRADING3B)\n");
    return the_dev->cd_ops->dirtypage(file, offset);
}
...
{
bytedev_t *the_dev = file->vn_cdev;
    KASSERT(the_dev != NULL && the_dev->cd_ops != NULL && the_dev->cd_ops->cleanpage != NULL);
    dbg(DBG_PRINT, "(GRADING3B)\n");
    return the_dev->cd_ops->cleanpage(file, offset, pagebuf);
}
