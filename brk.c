...
{
        vmmap_t *process_vmmap;
        vmarea_t *heap_vmarea;
        uint32_t new_end_vfn;
        uint32_t current_start_vfn;
        uint32_t page_count;
        int range_is_empty;
        ...
        if (addr < curproc->p_start_brk || 
            (unsigned int)addr > USER_MEM_HIGH) {
                dbg(DBG_PRINT, "(GRADING3E)\n");
                return -ENOMEM;
        }

        process_vmmap = curproc->p_vmmap;
        new_end_vfn = ((uint32_t)addr - 1) / PAGE_SIZE + 1;
        current_start_vfn = ((uint32_t)curproc->p_brk - 1) / PAGE_SIZE + 1;

        if (new_end_vfn > current_start_vfn) {
                dbg(DBG_PRINT, "(GRADING3B)\n");

                heap_vmarea = vmmap_lookup(process_vmmap, current_start_vfn - 1);
                if (heap_vmarea == NULL) {
                        dbg(DBG_PRINT, "(GRADING3E)\n");
                        return -ENOMEM;
                }

                page_count = new_end_vfn - current_start_vfn;
                range_is_empty = vmmap_is_range_empty(process_vmmap, 
                                                      current_start_vfn, page_count);
                ...
                dbg(DBG_PRINT, "(GRADING3B)\n");
                heap_vmarea->vma_end = new_end_vfn;
        } else if (new_end_vfn < current_start_vfn) {
                dbg(DBG_PRINT, "(GRADING3E)\n");
                page_count = current_start_vfn - new_end_vfn;
                vmmap_remove(process_vmmap, new_end_vfn, page_count);
        }
