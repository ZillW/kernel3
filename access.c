...
{
        vmarea_t *target_vmarea;
        uint32_t virtual_page_num;
        int area_protection_flags;
        int has_read_perm;
        int has_write_perm;
        int has_exec_perm;

        virtual_page_num = ADDR_TO_PN(vaddr);
        target_vmarea = vmmap_lookup(p->p_vmmap, virtual_page_num);
        ...
        area_protection_flags = target_vmarea->vma_prot;
        has_read_perm = (perm & PROT_READ) != 0;
        has_write_perm = (perm & PROT_WRITE) != 0;
        has_exec_perm = (perm & PROT_EXEC) != 0;
        
        if (has_read_perm && (area_protection_flags & PROT_READ) == 0) {
                dbg(DBG_PRINT, "(GRADING3D)\n");
                return 0;
        }
        
        if (has_write_perm && (area_protection_flags & PROT_WRITE) == 0) {
                dbg(DBG_PRINT, "(GRADING3D)\n");
                return 0;
        }
        
        if (has_exec_perm && (area_protection_flags & PROT_EXEC) == 0) {
                dbg(DBG_PRINT, "(GRADING3D)\n");
                return 0;
        }
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
        ...
        while (current_offset < len) {
                current_address = (const void *)((uint32_t)avaddr + current_offset);
                permission_check_result = addr_perm(p, current_address, perm);
                
                if (permission_check_result == 0) {
                        dbg(DBG_PRINT, "(GRADING3D)\n");
                        return 0;
                }
