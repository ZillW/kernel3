...
sys_read(read_args_t *arg)
{
        ...
        copy_result = copy_from_user(&kernel_read_args, arg, sizeof(kernel_read_args));
        if (copy_result < 0) {
                curthr->kt_errno = -copy_result;
                return -1;
        }

        kernel_buffer = page_alloc();
        if (kernel_buffer == NULL) {
                curthr->kt_errno = ENOMEM;
                return -1;
        }

        read_result = do_read(kernel_read_args.fd, kernel_buffer, kernel_read_args.nbytes);
        if (read_result < 0) {
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                curthr->kt_errno = -read_result;
                page_free(kernel_buffer);
                return -1;
        }
        ...
        copy_result = copy_to_user(arg->buf, kernel_buffer, bytes_read);
        if (copy_result < 0) {
                curthr->kt_errno = -copy_result;
                page_free(kernel_buffer);
                return -1;
...
{
        write_args_t kernel_write_args;
        void *kernel_buffer;
        int copy_result;
        int write_result;
        int bytes_written;
        ...
        copy_result = copy_from_user(&kernel_write_args, arg, sizeof(kernel_write_args));
        if (copy_result < 0) {
                curthr->kt_errno = -copy_result;
                return -1;
        }
        ...
        copy_result = copy_from_user(kernel_buffer, kernel_write_args.buf, kernel_write_args.nbytes);
        if (copy_result < 0) {
                curthr->kt_errno = -copy_result;
                page_free(kernel_buffer);
                return -1;
        ...
        write_result = do_write(kernel_write_args.fd, kernel_buffer, kernel_write_args.nbytes);
        if (write_result < 0) {
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                curthr->kt_errno = -write_result;
                page_free(kernel_buffer);
                return -1;
        }
...
{
        getdents_args_t kernel_dirents_args;
        ...
        int getdent_result;
        int dirent_index;
        void *user_buffer_ptr;

        dbg(DBG_PRINT, "(GRADING3D)\n");

        copy_result = copy_from_user(&kernel_dirents_args, arg, sizeof(getdents_args_t));
        if (copy_result < 0) {
                curthr->kt_errno = -copy_result;
                return -1;
        }

        max_dirent_count = (int)kernel_dirents_args.count / sizeof(dirent_t);
        total_bytes_copied = 0;

        for (dirent_index = 0; dirent_index < max_dirent_count; dirent_index++) {
                getdent_result = do_getdent(kernel_dirents_args.fd, &current_dirent);

                if (getdent_result < 0) {
                        dbg(DBG_PRINT, "(GRADING3D)\n");
                        curthr->kt_errno = -getdent_result;
                        return -1;
                } else if (getdent_result == 0) {
                        dbg(DBG_PRINT, "(GRADING3B)\n");
                        break;
                }

                dbg(DBG_PRINT, "(GRADING3B)\n");
                user_buffer_ptr = (void *)((char *)kernel_dirents_args.dirp + total_bytes_copied);
                copy_result = copy_to_user(user_buffer_ptr, &current_dirent, getdent_result);
                if (copy_result < 0) {
                        curthr->kt_errno = -copy_result;
                        return -1;
                }
                total_bytes_copied += getdent_result;
        }
